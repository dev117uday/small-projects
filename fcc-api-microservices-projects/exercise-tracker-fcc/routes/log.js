...
router.route("/log").get(async function (req, res) {
  try {
    // get query parameter
    ...
    if (getUserId) {
      ...
      !exerciseQuery.date.hasOwnProperty("$gte") &&
        !exerciseQuery.date.hasOwnProperty("$lte") &&
        delete exerciseQuery.date;

      const findExercise = await Exercise.find(exerciseQuery, {
        description: 1,
        duration: 1,
        date: 1,
        _id: 0,
